...
In red, this is the algorithm available in the first version of the package.
Now, to use it, you have to set `joint.align = FALSE` and `quantif.method = "FWER"`.
To perform a joint alignment (blue, green and yellow scenarios), `joint.align`
need to be set to `TRUE`. To perform a joint quantification just after the
alignment use `joint.align = "Lasso"`. Finally, to perform a joint quantification
only on metabolites identified with the independent quantification use 
`joint.align = "both"`.

With `joint.align = "both"`, it also possible to limit the number of 
identified metabolites using `clean.thres`. If `clean.thres = 10`, only the
metabolites identified in at least 10% of the spectra identified.
